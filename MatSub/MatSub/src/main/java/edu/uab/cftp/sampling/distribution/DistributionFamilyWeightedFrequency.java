...
public class DistributionFamilyWeightedFrequency extends
	...
	public static class Tester extends DistributionTester {
		...
		public double computeMeasureValue(PlainItemSet itemSet) {
			...
			double supportMinusOneCardinality = getSupportMinusOneCardinality(itemSet);

			return weightedSupport * supportMinusOneCardinality;
		}

		private double getSupportMinusOneCardinality(PlainItemSet itemSet) {
			return Math.pow(itemSet.getTIDs().cardinality(), cardinality - 1);
		}
	...
	public DistributionFamilyWeightedFrequency(
	...
	public double baseObjectWeight(OrderedBaseObject baseObject) {
		if (baseObject.getWeight() != -1) {
			...
			for (int tid : baseObject.getIndices()) {
				weight += getWeightInDistribution(transactionIntersection,
						wdb.getTransactionWeight(tid));
			}
